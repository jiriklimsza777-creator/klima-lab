...
def _render_topbar_controls():
    """
    One-line top bar:
      Producenti | Nástroje | Počet | Takty | Hustota | BPM | Témata
    ...
    # Top bar picker: show only even bar counts (user preference).
    bars_opts = list(range(2, 17, 2))
    with cols[3]:
        if hasattr(st, "popover"):
    ...
    # BPM as a slide-out picker (replaces Role in topbar).
    cur_bpm = int(st.session_state.get("bpm") or 90)
    cur_bpm = max(40, min(220, cur_bpm))
    bpm_opts = list(range(40, 221, 5))
    with cols[5]:
        if hasattr(st, "popover"):
            with st.popover(f"BPM: {cur_bpm} ▾", use_container_width=True):
                picked = st.radio(
                    "Vyber BPM",
                    bpm_opts,
                    index=bpm_opts.index(cur_bpm) if cur_bpm in bpm_opts else bpm_opts.index(90),
                    horizontal=False,
                    key="top_bpm_pick",
                    label_visibility="collapsed",
                )
                if picked and int(picked) != int(cur_bpm):
                    st.session_state["bpm_pending"] = int(picked)
                    st.rerun()
        else:
            picked = st.selectbox(
                "BPM",
...
def _split_dataset_lead_chords(notes):
    ...
    # Overlap graph (single-track friendly): notes connected by time overlap.
    n = len(norm)
    parent = list(range(n))
    ...
    # Phase A/B: split overlap groups by register (higher -> lead, lower -> chords).
    for idxs in groups.values():
        if len(idxs) <= 1:
            isolated.extend(idxs)
            continue
        grp = [norm[i] for i in idxs]
        grp_sorted = sorted(grp, key=lambda x: (x["pitch"], x["dur"], x["i"]))
        split_k = max(1, len(grp_sorted) // 2)
        low = grp_sorted[:split_k]
        high = grp_sorted[split_k:]
        if not high:
            high = grp_sorted[-1:]
            low = grp_sorted[:-1]
        chords_raw.extend(low)
        lead_raw.extend(high)

    # Phase C: isolated notes split by P75 threshold.
    iso_notes = [norm[i] for i in isolated]
    if iso_notes:
        iso_pitches = sorted(int(x["pitch"]) for x in iso_notes)
        if len(iso_pitches) == 1:
            threshold = float(iso_pitches[0])
        else:
            q = 0.75 * (len(iso_pitches) - 1)
            lo = int(np.floor(q))
            hi = int(np.ceil(q))
            if lo == hi:
                threshold = float(iso_pitches[lo])
            else:
                frac = float(q - lo)
                threshold = float(iso_pitches[lo]) + (float(iso_pitches[hi]) - float(iso_pitches[lo])) * frac
        for x in iso_notes:
            if float(x["pitch"]) > float(threshold):
                lead_raw.append(x)
            else:
                chords_raw.append(x)  # threshold-equal goes to chords by design

    # Fallbacks: ensure both branches have content.
    if not lead_raw and chords_raw:
        ch_sorted = sorted(chords_raw, key=lambda x: (x["pitch"], x["dur"], x["i"]))
        take = max(1, len(ch_sorted) // 4)
        moved = ch_sorted[-take:]
        moved_ids = {m["i"] for m in moved}
        lead_raw.extend(moved)
        chords_raw = [x for x in chords_raw if x["i"] not in moved_ids]
    if not chords_raw and lead_raw:
        ld_sorted = sorted(lead_raw, key=lambda x: (x["pitch"], x["dur"], x["i"]))
        take = max(1, len(ld_sorted) // 4)
        moved = ld_sorted[:take]
        moved_ids = {m["i"] for m in moved}
        chords_raw.extend(moved)
        lead_raw = [x for x in lead_raw if x["i"] not in moved_ids]

    # lead: monophonic-ish, keep highest note per 1/4 beat bucket
    lead_map = {}
    for x in lead_raw:
        t = round(float(x["start"]) / 0.25) * 0.25
        cur = lead_map.get(t)
        if cur is None:
            lead_map[t] = x
            continue
        # Stable tie-break: higher pitch > shorter dur > earlier source index.
        better = (
            int(x["pitch"]) > int(cur["pitch"])
            or (
                int(x["pitch"]) == int(cur["pitch"])
                and (
                    float(x["dur"]) < float(cur["dur"])
                    or (float(x["dur"]) == float(cur["dur"]) and int(x["i"]) < int(cur["i"]))
                )
            )
        )
        if better:
    ...
    lead = sorted(
        [
            [round(float(k), 4), int(v["pitch"]), float(v["dur"]), int(v["vel"])]
            for k, v in lead_map.items()
        ],
        key=lambda x: (x[0], x[1]),
    )
    chords = sorted(
        [[float(x["start"]), int(x["pitch"]), float(x["dur"]), int(x["vel"])] for x in chords_raw],
        key=lambda x: (x[0], x[1]),
    )
    return lead, chords
...
def _apply_local_density(notes, density: str, bars: int):
    ...
    def _pick_spread(chunk, limit: int):
        """
        Keep notes distributed through the whole bar instead of only earliest ones.
        This avoids long silent tails at the end of each bar after thinning.
        """
        if len(chunk) <= limit:
            return list(chunk)
        if limit <= 1:
            return [chunk[0]]
        n = len(chunk)
        idxs = [int(round(i * (n - 1) / float(limit - 1))) for i in range(limit)]
        seen = set()
        out = []
        for idx in idxs:
            if idx in seen:
    ...
    if density == "Méně not":
        ...
        for n in notes_sorted:
            ...
            if 0 <= bar < bars:
                by_bar.setdefault(bar, []).append(n)
        for b in range(bars):
            out.extend(_pick_spread(by_bar.get(b, []), per_bar))
        out.sort(key=lambda x: (float(x[0]), int(x[1])))
        return out
...
def _seeded_call(seed: int, fn):
    # Keep random behavior reproducible per-candidate without polluting global RNG state.
...
def _score_targets(selected_producer: str, theme_string: str):
    producer_raw = str(selected_producer or "").strip()
    _, theme_name = split_theme(str(theme_string or ""))
    producer_profile = get_producer_profile(producer_raw)
    theme_profile = build_theme_profile(theme_name)

    density_base = float(producer_profile.get("density", 0.68))
    density_theme_mult = float(theme_profile.get("density", 1.0))
    density_eff = max(0.45, min(1.15, density_base * density_theme_mult))
    target_notes_per_bar = max(2.0, min(5.8, 1.6 + (density_eff * 2.4)))

    reg_lo, reg_hi = producer_profile.get("register", (52, 70))
    reg_shift = int(theme_profile.get("register_shift", 0))
    reg_lo = int(reg_lo) + reg_shift
    reg_hi = int(reg_hi) + reg_shift
    reg_width = max(6, int(reg_hi - reg_lo))
    target_range = max(7.0, min(16.0, float(reg_width) * 0.42))
    target_center = float(reg_lo + reg_hi) / 2.0
    return {
...
def _evaluate_melody(
    ...
    *,
    ...
    theme_string: str = "",
    producer_profile: dict | None = None,
    theme_profile: dict | None = None,
):
    """
    Lightweight quality score for monophonic-ish loop melodies.
    Higher is better.
    """
    if not notes:
        return -9999.0
    try:
        notes_sorted = sorted(
            [
                [float(n[0]), int(n[1]), float(n[2]), int(n[3]) if len(n) > 3 else 90]
                for n in notes
    ...
    total_beats = max(1.0, float(int(bars) * 4.0))
    count = len(notes_sorted)
    pitches = [n[1] for n in notes_sorted]
    starts = [n[0] for n in notes_sorted]

    if producer_profile is None:
        producer_profile = get_producer_profile(str(selected_producer or ""))
    if not isinstance(producer_profile, dict):
        producer_profile = dict(DEFAULT_PROFILE)
    if theme_profile is None:
        _, _theme_name = split_theme(str(theme_string or ""))
        theme_profile = build_theme_profile(_theme_name)
    if not isinstance(theme_profile, dict):
        theme_profile = build_theme_profile("")

    density_base = float(producer_profile.get("density", DEFAULT_PROFILE.get("density", 0.68)))
    target_notes_per_bar = max(1.5, min(7.0, density_base * 7.0))

    reg_lo, reg_hi = producer_profile.get("register", DEFAULT_PROFILE.get("register", (52, 70)))
    reg_shift = int(theme_profile.get("register_shift", 0))
    reg_lo = int(reg_lo) + reg_shift
    reg_hi = int(reg_hi) + reg_shift
    reg_width = max(6, int(reg_hi - reg_lo))
    target_range = max(6.0, min(18.0, float(reg_width) * 0.45))
    target_center = float(reg_lo + reg_hi) / 2.0

    # Density target is producer/theme-aware (instead of one global ideal).
    notes_per_bar = count / max(1.0, total_beats / 4.0)
    density_score = -abs(notes_per_bar - target_notes_per_bar) * 5.0

    # Penalize large pitch jumps.
    jumps = [abs(pitches[i] - pitches[i - 1]) for i in range(1, len(pitches))]
    mean_jump = (sum(jumps) / len(jumps)) if jumps else 0.0
    jump_target = float(producer_profile.get("jump", DEFAULT_PROFILE.get("jump", 5)))
    jump_score = -max(0.0, mean_jump - jump_target) * 2.2
    big_jump_pen = -sum(1 for j in jumps if j >= max(10, int(jump_target + 4))) * 1.5

    # Range control: too narrow boring, too wide chaotic.
    pitch_range = (max(pitches) - min(pitches)) if pitches else 0
    range_score = -abs(float(pitch_range) - target_range) * 0.7
    mean_pitch = (sum(pitches) / len(pitches)) if pitches else target_center
    center_score = -abs(float(mean_pitch) - float(target_center)) * 0.25

    # Repetition/motif score: duplicates of (time modulo bar, pitch-class-ish) are good.
    motif_bins = {}
    for t, p, *_ in notes_sorted:
        key = (round(float(t) % 4.0, 2), int(p) % 12)
        motif_bins[key] = motif_bins.get(key, 0) + 1
    repeated = sum(1 for v in motif_bins.values() if v >= 2)
    motif_score = repeated * 0.8

    # Loop closure: ending note should relate to the beginning.
    first_p = pitches[0]
    last_p = pitches[-1]
    loop_score = max(0.0, 8.0 - abs(last_p - first_p)) * 0.7

    # Rhythm balance: too many identical starts (stacking) is bad for lead loop.
    unique_starts = len({round(float(t), 3) for t in starts})
    uniq_ratio = unique_starts / max(1, count)
    uniq_score = -abs(uniq_ratio - 0.92) * 6.0

    # Duration fit: prefer phrases that resemble producer rhythm palette.
    producer_durs = [float(d) for d in (producer_profile.get("durations") or DEFAULT_PROFILE.get("durations") or [0.25, 0.5, 1.0])]
    avg_dur = sum(float(n[2]) for n in notes_sorted) / max(1, len(notes_sorted))
    target_avg_dur = sum(producer_durs) / max(1, len(producer_durs))
    dur_score = -abs(float(avg_dur) - float(target_avg_dur)) * 4.0

    return float(density_score + jump_score + big_jump_pen + range_score + center_score + motif_score + loop_score + uniq_score + dur_score)


def _generate_best_random_candidate(
    selected_producer,
    theme_string,
    bars,
    ai_prompt,
    ...
    *,
    ...
    base_seed = int(st.session_state.get("random_seed_base") or 0)
    if base_seed <= 0:
        base_seed = random.randint(10_000, 9_999_999)
        st.session_state["random_seed_base"] = int(base_seed)

    best = None
    best_score = -1e18
    best_seed = base_seed
    # Each visible variant must search in a different seed window,
    # otherwise multiple cards in one batch can collapse to the same melody.
    window = max(10, int(candidate_count) * 10)
    start_seed = int(base_seed + (int(variant_offset) * window))
    for i in range(candidate_count):
        seed_i = int(start_seed + i)

        cand = _seeded_call(
            seed_i,
            lambda: _generate_notes(selected_producer, theme_string, bars, ai_prompt, chords_mode),
        )
        score = _evaluate_melody(
            cand or [],
            int(bars),
            selected_producer=str(selected_producer or ""),
            theme_string=str(theme_string or ""),
        )
        if score > best_score:
            best = cand
            best_score = score
    ...
    return best or [], int(best_seed), float(best_score)


def _generate_best_dataset_candidate(
    bars: int,
    *,
    candidate_count=5,
    variant_offset=0,
    ...
    window = max(10, int(candidate_count) * 10)
    start_seed = int(base_seed + (int(variant_offset) * window))
    best_ds = {"melody": [], "layers": []}
    best_score = -1e18
    best_seed = start_seed
    variation_on = bool(st.session_state.get("dataset_more_variation", False))
    recent_sigs = st.session_state.get("_dataset_recent_signatures")
    if not isinstance(recent_sigs, list):
        recent_sigs = []
    for i in range(candidate_count):
        seed_i = int(start_seed + i)
        ds = _seeded_call(seed_i, lambda: _generate_from_dataset(int(bars)))
        mel = ds.get("melody", []) if isinstance(ds, dict) else []
        score = _evaluate_melody(
            mel or [],
            int(bars),
            selected_producer=str(selected_producer or ""),
        ...
        if variation_on and mel:
            sig = _dataset_signature(mel)
            if sig:
                # Soft anti-repeat penalty (only when toggle is ON).
                same_hits = sum(1 for s in recent_sigs[-8:] if s == sig)
                if same_hits > 0:
                    score -= float(1.2 * same_hits)
        if score > best_score:
            best_ds = ds if isinstance(ds, dict) else {"melody": [], "layers": []}
            best_score = score
            best_seed = seed_i
    if variation_on:
        best_mel = best_ds.get("melody", []) if isinstance(best_ds, dict) else []
        best_sig = _dataset_signature(best_mel)
        if best_sig:
            recent_sigs.append(best_sig)
            st.session_state["_dataset_recent_signatures"] = recent_sigs[-24:]
    return best_ds, int(best_seed), float(best_score)
...
def render_import_page(selected_producer, selected_theme):
    ...
    st.title("MAX FIDELITY WAV -> MIDI")
    uploaded = st.file_uploader("Nahraj beat (WAV nebo MP3):", type=["wav", "mp3"])
    if uploaded is None:
        st.info("Nahraj jednodušší loop/sample a appka z něj udělá jeden MIDI výstup.")
        return
    ...
    st.markdown("---")
    st.subheader("Převod")
    st.caption("RAW Basic Pitch 1:1. Bez přepočtu na BPM/takty z generátoru.")
    ...
    if st.button("🧠 MAX FIDELITY TRANSCRIBE", use_container_width=True):
        wav_bytes = st.session_state.get("import_beat_wav_bytes") or b""
        if not wav_bytes:
            st.warning("Nejdřív nahraj WAV/MP3 beat.")
        else:
            with st.spinner("Analyzuju audio (multi-pass) a připravuju finální MIDI..."):
                data = sound.export_max_fidelity_midi(
                    wav_bytes=wav_bytes,
                    bpm=90,
                    bars=4,
                )
            st.session_state.boombap_extract_tight_midi = data.get("midi_bytes") or b""
            st.session_state.boombap_extract_tight_notes = data.get("notes") or []
            st.session_state.boombap_extract_loose_midi = b""
            st.session_state.boombap_extract_loose_notes = []
            if st.session_state.boombap_extract_tight_midi:
                st.toast("Finální MIDI hotové.")
            else:
                st.warning("Nepodařilo se najít stabilní noty. Zkus BPM nebo kratší/čistší loop.")

    final_notes = st.session_state.get("boombap_extract_tight_notes") or []
    final_midi = st.session_state.get("boombap_extract_tight_midi") or b""
    if final_midi:
        st.markdown(f"**Extracted notes:** `{len(final_notes)}`")
        try:
            notes_preview, bpm_preview, _ = sound.import_midi_bytes(final_midi)
            if notes_preview:
                if st.session_state.engine_type == ENGINE_STUDIO:
                    instr_id = STUDIO_INSTRUMENTS.get(str(st.session_state.import_main_instrument), 0)
                else:
                    instr_id = LAB_WAVES.get(str(st.session_state.import_main_instrument), 0)
                preview_wav = sound.synthesise_full_audio(
                    notes_preview,
                    engine_type=st.session_state.engine_type,
                    instrument_id=int(instr_id),
                    octave_shift=0,
                    bpm=int(max(40, min(220, int(bpm_preview or 90)))),
                )
                if preview_wav:
                    st.markdown("**Náhled výsledku (v importu):**")
                    st.audio(preview_wav, format="audio/wav")
        except Exception as e:
            st.warning(f"Nepovedlo se vytvořit audio náhled z finálního MIDI: {e}")
        st.download_button(
            "Stáhnout finální MIDI",
            data=final_midi,
            file_name="max_fidelity_final.mid",
            mime="audio/midi",
            use_container_width=True,
        )

        st.markdown("### Otevřít / Uložit jako projekt")
        title_base = st.text_input("Název projektu", value="Max Fidelity Extract", key="import_extract_title")
        imp_theme = st.text_input("Téma", value=str(theme_string or "Imported Reference"), key="import_extract_theme")

        final_project = create_project(
            str(title_base or "Max Fidelity Extract"),
            str(imp_theme or "Imported Reference"),
            final_notes,
            instrument_name=str(st.session_state.import_main_instrument or "Acoustic Grand Piano"),
            source="imported",
            bars=max(1, int(round((max([n[0] + n[2] for n in final_notes]) if final_notes else 4) / 4.0))),
            locked=False,
            humanize="Off",
        ...
        with act_cols[0]:
            if st.button("Otevřít v generátoru", use_container_width=True, disabled=not bool(final_notes)):
                _n, bpm_guess, _i = sound.import_midi_bytes(final_midi)
                st.session_state["bpm_pending"] = int(max(40, min(220, int(bpm_guess or 90))))
                open_project_from_archive(final_project)
        with act_cols[1]:
            if st.button("Uložit do archivu", use_container_width=True, disabled=not bool(final_notes)):
                _n, bpm_guess, _i = sound.import_midi_bytes(final_midi)
                db.save_to_db(
                    final_project["title"],
                    final_project["theme"],
                    final_project["main_instrument"],
                    final_project,
                    int(max(40, min(220, int(bpm_guess or 90)))),
                    0,
                    tags="",
                    source_type="imported",
                )
                st.toast("Projekt uložen do Archivu.")
...
def generate_projects(selected_producer, final_theme_string, ai_prompt, chords_mode, progress_ui=None):
    ...
    engine_mode = str(st.session_state.get("generation_engine") or "")
    dataset_mode = engine_mode == "Dataset Style"
    local_random_mode = engine_mode == "Lokální (Random)"
    if local_random_mode or dataset_mode:
        # New batch => new seed space so repeated "GENEROVAT" does not clone the same set.
        st.session_state["random_seed_base"] = random.randint(10_000, 9_999_999)
    for index in range(total):
        ...
        layers_for_project = []
        if dataset_mode:
            ds, used_seed, quality_score = _generate_best_dataset_candidate(
                int(bars_for_mode),
                candidate_count=int(st.session_state.get("random_topk_candidates") or 5),
                variant_offset=int(index),
                selected_producer=str(selected_producer or ""),
                theme_string=str(final_theme_string or ""),
            )
            ...
            if local_random_mode and (not chords_mode):
                result, used_seed, quality_score = _generate_best_random_candidate(
                    selected_producer,
                    final_theme_string,
                    bars_for_mode,
                    ai_prompt,
        ...
        if projects:
            try:
                if (local_random_mode or dataset_mode) and (not chords_mode) and (not _is_any_solo_mode()):
                    projects[-1]["generation_seed"] = int(used_seed) if used_seed is not None else None
                    projects[-1]["quality_score"] = float(quality_score) if quality_score is not None else None
                    projects[-1]["candidate_count"] = int(st.session_state.get("random_topk_candidates") or 5)
            except Exception as e:
                st.info(f"Nepovedlo se uložit metadata seed/quality: {e}")
        ...
        if _is_any_solo_mode() and projects:
            try:
                ...
                st.info(f"Nepovedlo se uložit solo metadata: {e}")
            ...
            if not project.get("locked"):
                ...
                if dataset_mode:
                    ds, used_seed, quality_score = _generate_best_dataset_candidate(
                        int(bars),
                        candidate_count=int(st.session_state.get("random_topk_candidates") or 5),
                        variant_offset=int(idx),
                    )
                    project["melody"] = ds.get("melody", []) or project.get("melody", [])
                    project["layers"] = list(ds.get("layers", []) or [])
                    project["generation_seed"] = int(used_seed)
                    project["quality_score"] = float(quality_score)
                    project["candidate_count"] = int(st.session_state.get("random_topk_candidates") or 5)
                else:
        ...
        for i in range(len(st.session_state.projects)):
            ...
            if dataset_mode:
                ds, used_seed, quality_score = _generate_best_dataset_candidate(
                    int(bars),
                    candidate_count=int(st.session_state.get("random_topk_candidates") or 5),
                    variant_offset=int(i),
                )
                p["melody"] = ds.get("melody", []) or p.get("melody")
                p["layers"] = list(ds.get("layers", []) or [])
                p["generation_seed"] = int(used_seed)
                p["quality_score"] = float(quality_score)
                p["candidate_count"] = int(st.session_state.get("random_topk_candidates") or 5)
            else:
