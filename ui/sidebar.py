...
def render_sidebar(all_producers, all_themes):
    ...
    with st.sidebar:
        ...
        engine_options = [ENGINE_RANDOM, ENGINE_DATASET, ENGINE_AI, ENGINE_SOLOS]
        ...
        mode_options = [MODE_MELODIC, MODE_CHORDS_UI, MODE_LOOP]
        ...
        with c_q2:
            st.markdown(
                ...
                "Solo nástroje přepneš nahoře přes Generátor.' "
        ...
        engine_now = str(st.session_state.get("generation_engine") or ENGINE_RANDOM)
        if engine_now == ENGINE_DATASET:
            ...
            st.toggle(
                "Více variability (Dataset)",
                key="dataset_more_variation",
                value=bool(st.session_state.get("dataset_more_variation", False)),
                help="OFF = stabilní výběr jako dosud. ON = jemně penalizuje opakující se návrhy.",
            )
            # Keep dataset mode simple: fixed theme influence (no user slider).
            st.session_state["dataset_theme_influence"] = 35
        ...
        elif engine_now == ENGINE_SOLOS:
            # Engine shortcut: selecting "Sola" here directly enters solo generation mode.
            st.session_state["gen_mode"] = MODE_SOLOS
            st.session_state.ai_chords = False
            ...
            st.caption("Solo mód je aktivní přes volbu Generátor → Solo nástroje.")
            gen_mode = MODE_SOLOS
        else:
            gen_mode = st.radio(
                "Co generovat:",
        ...
        with st.expander("PARAMETRY SKLADBY", expanded=False):
            ...
            if str(st.session_state.get("generation_engine") or "") in {ENGINE_RANDOM, ENGINE_DATASET}:
                st.selectbox(
                    "Počet interních pokusů (kvalita):",
                    [3, 5, 7, 10],
                    index=[3, 5, 7, 10].index(int(st.session_state.get("random_topk_candidates") or 5))
                    if int(st.session_state.get("random_topk_candidates") or 5) in [3, 5, 7, 10]
                    else 1,
                    key="random_topk_candidates",
                    help=(
                        "Kolik interních návrhů Random generátor zkusí, než vybere nejlepší.\n"
                        "Kdy: Když chceš stabilně lepší kvalitu.\n"
                        "Dopad: Vyšší číslo = obvykle lepší výsledek, ale delší čekání."
                    ),
                )
