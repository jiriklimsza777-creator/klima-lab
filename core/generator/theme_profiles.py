...
THEME_ALIAS_MAP = {
    ...
    "sad piano": "Melancholic",
    "rainy night": "Melancholic",
    "lo-fi chill": "Lo-Fi Chill",
    "hard boom bap": "Hard Boom Bap",
    "jazzy": "Jazzy",
    "soulful": "Jazzy",
    "dreamy": "Dreamy",
...
def _resolve_theme_label(theme_name: str) -> str:
    t = str(theme_name or "").strip().lower()
    if not t:
        return ""
    for old, new in THEME_ALIAS_MAP.items():
        if t == old or old in t:
            return str(new)
    return str(theme_name or "").strip()
...
def build_theme_profile(theme_name):
    ...
    canonical_rules = {
        # Dark = Dark + Aggressive + Street
        "Dark": {"scale": [0, 1, 5, 7, 8], "density": 0.96, "durations": [0.25, 0.5, 1.0, 1.5], "jump": 1, "register_shift": -2, "accent": 3, "chords": [[0, 3, 7], [0, 3, 6], [0, 1, 7], [0, 5, 10]], "progression": [0, 1, 5, 8]},
        # Jazzy = Jazzy + Soulful
        "Jazzy": {"scale": [0, 3, 5, 7, 9, 10], "density": 0.93, "swing": 1.22, "durations": [0.5, 0.75, 1.0, 1.5], "jump": 1, "register_shift": 3, "chords": [[0, 3, 7, 10], [0, 5, 7, 10], [0, 3, 7, 9], [0, 5, 9]], "progression": [0, 3, 5, 9]},
        "Lo-Fi Chill": {"density": 0.75, "swing": 1.2, "durations": [0.5, 1.0, 1.5], "jump": -1, "register_shift": 1, "chords": [[0, 3, 7, 10], [0, 5, 7], [0, 3, 7, 9]]},
        "Hard Boom Bap": {"density": 0.82, "swing": 1.1, "durations": [0.5, 0.5, 1.0], "jump": -1, "register_shift": -1, "accent": 6, "chords": [[0, 3, 7], [0, 5, 10], [0, 3, 6]]},
        ...
        # Melancholic = Melancholic + Sad Piano + Rainy Night
        "Melancholic": {"scale": [0, 1, 3, 5, 7, 10], "density": 0.75, "durations": [0.5, 1.0, 1.5, 2.0], "register_shift": 3, "chords": [[0, 3, 7], [0, 3, 10], [0, 7, 10], [0, 5, 10]], "progression": [0, 1, 7, 10]},
    }

    theme_resolved = _resolve_theme_label(theme_name)
    theme_norm = str(theme_resolved or "").strip().lower()
    for label, update in canonical_rules.items():
