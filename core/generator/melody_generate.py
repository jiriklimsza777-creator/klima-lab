...
from core.generator.utils import build_allowed_notes, choose_next_pitch, merge_unique, weighted_choice
...
def smart_generate(
    ...
    if style_s.startswith("klas") or style_s in {"classic", "legacy", "old"}:
        return smart_generate_classic(num_bars, theme=theme, energy=energy)
    if style_s.startswith("sax"):
        return generate_sax_solo(
            theme=theme,
            bars=int(num_bars),
            energy=energy,
    ...
    if style_s.startswith("vib") or style_s.startswith("vibra"):
        return generate_vibraphone_solo(
            theme=theme,
            bars=int(num_bars),
            energy=energy,
    ...
    if style_s.startswith("akust") or style_s.startswith("acoustic bass") or style_s.startswith("acoustic_bass"):
        return generate_acoustic_bass_solo(
            theme=theme,
            bars=int(num_bars),
            energy=energy,
    ...
    allowed = build_allowed_notes(root, theme_profile["scale"], register)
    if not allowed:
        allowed = build_allowed_notes(60, [0, 3, 5, 7, 10], (52, 72))
    ...
    motif_length = weighted_choice(producer_profile["motif"], 2.0)
    ...
    durations = merge_unique(producer_profile["durations"] + theme_profile["durations"])
    ...
    if boombap_mode and motif_length >= 4.0:
        ...
        for t in chosen_call:
            ...
            if last_pitch is None and strong_beat:
                ...
                pitch = choose_next_pitch(last_pitch, allowed, jump_limit)

            if strong_beat:
                duration = weighted_choice([1.0, 1.5, 2.0] + durations, 1.0)
                velocity = random.randint(74, 108) + 18 + accent_boost
            else:
                duration = weighted_choice([0.25, 0.5, 0.5, 1.0] + durations, 0.5)
        ...
        for start, pitch, dur, vel in call_notes:
            ...
            if random.random() < 0.35:
                dur2 = float(weighted_choice([0.25, 0.5, 1.0] + durations, dur2))
        ...
        if vv_eff >= 0.65:
            ...
            if tag_t >= phrase_len and all(abs(float(n[0]) - tag_t) > 1e-6 for n in response_notes):
                ...
                response_notes.append([tag_t, tag_pitch, float(weighted_choice([0.25, 0.5], 0.25)), int(max(1, min(127, 96 + accent_boost)))])

        end_t = float(round(motif_length - step, 3))
        end_prob = 0.55 + (0.35 * (1.0 - vv))
        if end_t > 0 and random.random() < end_prob:
            response_notes.append([end_t, int(random.choice(cadence_candidates)), float(weighted_choice([0.5, 1.0, 1.5], 1.0)), int(max(1, min(127, 92 + accent_boost)))])
        ...
        for t in chosen_times:
            ...
            if strong_beat:
                duration = weighted_choice([1.0, 1.5, 2.0] + durations, 1.0) if boombap_mode else weighted_choice(durations, 0.5)
                velocity = random.randint(74, 108) + (18 if boombap_mode else 10) + accent_boost
            else:
                duration = weighted_choice([0.25, 0.5, 0.5, 1.0] + durations, 0.5) if boombap_mode else weighted_choice(durations, 0.5)
                velocity = random.randint(58, 98) + accent_boost
            ...
            if start_time < num_bars * 4:
                ...
                if can_mutate:
                    if variation_roll < (0.10 if boombap_mode else 0.18):
                        new_pitch = choose_next_pitch(note[1], allowed, max(2, jump_limit - 1))
                    if variation_roll > (0.90 if boombap_mode else 0.82):
                        new_duration = weighted_choice(durations, note[2])
                data.append([float(round(start_time, 3)), int(new_pitch), float(new_duration), int(note[3])])

    data.sort(key=lambda x: x[0])

    # Keep all notes strictly inside the generated arrangement length.
    # This avoids tail-overflow notes in loop-heavy modes (e.g. Boombap Loop).
    max_time = float(max(1, int(num_bars)) * 4)
    clipped = []
    for n in data:
        try:
            start = float(n[0])
            ...
            continue
        if dur <= 0.0 or start < 0.0 or start >= max_time:
            continue
        end = start + dur
        if end > max_time:
            dur = max(0.05, max_time - start)
        if dur <= 0.0:
            continue
        clipped.append([float(round(start, 3)), pitch, float(round(dur, 3)), int(max(1, min(127, vel)))])
    return clipped
