...
def init_state():
    ...
    defaults = {
        "page": PAGE_GENERATOR,
        "selected_producer": "Ĺ˝ĂˇdnĂ˝ (vypnuto)",
        "selected_theme": "Ĺ˝ĂˇdnĂ© (vypnuto)",
        "import_audio_sig": "",
        ...
        "boombap_variation_level": "StĹ™ednÄ›",
        # Sax solo controls (only used when local_melody_style starts with "Sax Solo")
        "sax_solo_character": "KlidnĂ©",  # KlidnĂ© / DivokĂ©
        "sax_solo_structure": "Bez pĹ™Ă­bÄ›hu",  # Bez pĹ™Ă­bÄ›hu / PĹ™Ă­bÄ›h
        "piano_solo_character": "KlidnĂ©",  # KlidnĂ© / DivokĂ©
        "piano_solo_structure": "Bez pĹ™Ă­bÄ›hu",  # Bez pĹ™Ă­bÄ›hu / PĹ™Ă­bÄ›h
        ...
        "bg_type": db.get_setting("default_bg_type", "VĂ˝chozĂ­ / ObrĂˇzek"),
        "card_style": db.get_setting("default_card_style", "KlasickĂ˝"),
        "chart_style": db.get_setting("default_chart_style", "KlasickĂ˝"),
...
def reset_project_widget_state(max_items: int):
    ...
    # we want the UI (especially the "NĂˇzev projektu" input) to reflect the new data.
