        ...
        with c_save:
            ...
            with st.popover(f"{stars_label} ▾", use_container_width=True):
                ...
                picked_rating = st.radio(
                    ...
                    key=f"{rating_key}_pick",
                    label_visibility="collapsed",
                    format_func=lambda x: "⭐" * int(x) + "✩" * (5 - int(x)),
                )
                if int(picked_rating) != current_rating:
                    st.session_state[rating_key] = int(picked_rating)
                    save_project_to_archive(project, rating=int(picked_rating))
                    st.rerun()
